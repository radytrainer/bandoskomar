(function ($) {
    ...
    $("#install-activate-button").on("click", function (e) {
        ...
        var activationData = {
            action: "charity_blocks_install_and_activate_plugins",
            nonce: charity_blocks_localize.nonce,
        };

        $.post(charity_blocks_localize.ajax_url, activationData, function (response) {
            console.log("asdasd", response);
            if (response.success) {
                window.location.href = charity_blocks_localize.redirect_url;
            } else {
    ...
    $(document).on('click', '.notice-info .notice-dismiss', function () {
        ...
        $.ajax({
            type: 'POST',
            url: charity_blocks_localize.ajax_url,
            data: {
                action: 'charity_blocks_dismissed_notice_handler',
                type: type,
                wpnonce: charity_blocks_localize.dismiss_nonce
            },
