 ...
 */
if (!function_exists('charity_blocks_is_plugin_installed')) {
    function charity_blocks_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('charity_blocks_is_plugin_activated')) {
    function charity_blocks_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_charity_blocks_install_and_activate_plugins', 'charity_blocks_install_and_activate_plugins');
add_action('wp_ajax_nopriv_charity_blocks_install_and_activate_plugins', 'charity_blocks_install_and_activate_plugins');
add_action('wp_ajax_charity_blocks_rplugin_activation', 'charity_blocks_rplugin_activation');
add_action('wp_ajax_nopriv_charity_blocks_rplugin_activation', 'charity_blocks_rplugin_activation');
...
function charity_blocks_all_plugins_active() {
...
function charity_blocks_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('charity_blocks_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_charity_blocks_plugin_installed($plugin_slug)) {
    ...
    $redirect_url = admin_url('themes.php?page=charity-blocks-guide-page');
    echo json_encode([
        'success' => true,
        'data' => [
...
function is_charity_blocks_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_charity_blocks_dismissed_notice_handler', 'charity_blocks_ajax_notice_dismiss_function');

function charity_blocks_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'charity_blocks_welcome_nonce')) {
...
function charity_blocks_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        ...
        if ($charity_blocks_current_screen && $charity_blocks_current_screen->id !== 'appearance_page_charity-blocks-guide-page') {
            ...
            <div class="charity-blocks-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="charity-blocks-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($charity_blocks_theme->get('Name')); ?></p>
                    ...
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" id="install-activate-button" href="#"><?php _e('Begin Installation', 'charity-blocks'); ?></a>`
                    </div>
                </div>
            </div>
            <?php
